  ...
  if (!response.ok) {
    try {
      ...
      if (contentType?.includes("application/json")) {
        ...
        logger.info("🔍 handleApiResponse: Error data received:", errorData);
...
export async function switchCompany(companyId: string): Promise<{
  ...
  try {
    logger.info("🔄 switchCompany: Making request to /api/v1/users/me", {
      companyId,
      API_URL,
    });
    ...
    logger.info("📡 switchCompany: Response received", {
    ...
    logger.error("❌ switchCompany: Exception caught:", error);
