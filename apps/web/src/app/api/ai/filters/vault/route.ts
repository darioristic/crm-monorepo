...
async function parseWithAI(query: string, availableTags: string[]): Promise<AIFilterResult> {
  if (!AI_CONFIG.apiKey) {
...
Today's date: ${today}`;
  ...
  try {
    ...
    logger.error("AI filter parsing error:", error);
    return { searchQuery: query };
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.error("AI filter endpoint error:", error);
