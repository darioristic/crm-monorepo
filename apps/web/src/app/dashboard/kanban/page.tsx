...
import { ArrowLeft, Calendar, ExternalLink, GripVertical, User as UserIcon } from "lucide-react";
...
function TaskCard({
  ...
  return (
    <Card className="cursor-grab active:cursor-grabbing" onClick={() => onTaskClick(task)}>
        ...
        <Kanban value={columns} onValueChange={handleValueChange} getItemValue={getItemValue}>
          <KanbanBoard className="min-w-max">
            {Object.entries(COLUMNS).map(([columnId, column]) => (
              <KanbanColumn key={columnId} value={columnId} className="w-[300px] bg-muted/50">
        ...
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              {selectedTask?.title}
              <Badge
                variant={priorityColors[selectedTask?.priority || "medium"]}
