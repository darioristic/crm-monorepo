...
export function useOrderSettings() {
  ...
  useEffect(() => {
    const fetchCompanyDetails = async () => {
      ...
      try {
        ...
        console.error("[useOrderSettings] Failed to fetch tenant company details:", error);
        setFromDetails(null);
        setLogoUrl(null);
      } finally {
        setIsLoading(false);
    ...
    try {
      const savedPaymentDetails = localStorage.getItem("order_payment_details");
  ...
  const defaultSettings = useMemo((): Partial<OrderDefaultSettings> => {
    return {
      template: {
        ...DEFAULT_ORDER_TEMPLATE,
        logoUrl: logoUrl || DEFAULT_ORDER_TEMPLATE.logoUrl,
      },
      fromDetails,
      paymentDetails,
    };
