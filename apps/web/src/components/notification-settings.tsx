...
export function NotificationSettings() {
  ...
  const { data: notificationTypes, isLoading } = useQuery<NotificationType[]>({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        settings = settingsResponse.success && settingsResponse.data ? settingsResponse.data : [];
      } catch {
        settings = [];
      }
  ...
  const types: NotificationType[] = notificationTypes ?? [];
  const groupedNotifications = types.reduce<Record<string, CategoryGroup>>(
    (acc: Record<string, CategoryGroup>, notificationType: NotificationType) => {
      ...
      // Get display info
      const displayInfo = getNotificationDisplayInfoWithFallback(notificationType.type);
  ...
  const sortedCategories: CategoryGroup[] = groupedNotifications
    ? Object.values(groupedNotifications).sort((a: CategoryGroup, b: CategoryGroup) => {
        if (a.order !== b.order) {
    ...
    <div className="space-y-6">
      {fetchError && (
        <Alert variant="destructive">
          <AlertTriangle />
          <AlertDescription>
            Failed to load notification settings. Showing default categories.
          </AlertDescription>
        </Alert>
      )}
