...
export function Form({ orderId, onSuccess, onDraftSaved }: FormProps) {
  ...
  useEffect(() => {
    ...
    if (!transformedData.companyId || !transformedData.issueDate || !transformedData.items || transformedData.items.length === 0) {
      return;
    }
  ...
  }, [orderId, debouncedValue, isDirty, orderNumberValid, form, onDraftSaved, transformFormValuesToDraft]);
    ...
    if (result.success && result.data) {
      ...
      toast.success(
        values.template.deliveryType === "create_and_send"
          ? isUpdate ? "Order updated and sent" : "Order created and sent"
