			...
			<DropdownMenu open={isActive} onOpenChange={setActive}>
				<DropdownMenuTrigger asChild>
					<Button variant="ghost" size="sm" className="h-9">
						...
						<span className="ml-2 font-semibold truncate text-muted-foreground text-sm">No Company</span>
		...
		<DropdownMenu open={isActive} onOpenChange={setActive}>
			<DropdownMenuTrigger asChild>
				<Button variant="ghost" size="sm" className="h-9" disabled={isChangingCompany}>
					...
					<span className="ml-2 font-semibold truncate text-sm max-w-[120px]">{selectedCompany.name}</span>
					<ChevronsUpDown className="ml-2 h-4 w-4" />
				</Button>
			</DropdownMenuTrigger>
			<DropdownMenuContent
