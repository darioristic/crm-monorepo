...
import { getCompanies, switchCompany } from "@/lib/companies";
import { logger } from "@/lib/logger";

export function CompanyHeaderDropdown() {
  ...
  const changeCompanyMutation = useMutation({
    mutationFn: async (companyId: string) => {
      ...
      logger.info("✅ Frontend: Switch successful");
      return result;
    },
    onSuccess: async (result) => {
      logger.info("✅ Frontend: Mutation success, refreshing...");
      ...
      if (result.data?.id) {
        await queryClient.invalidateQueries({
          queryKey: ["companies", result.data.id],
        });
      }
      ...
      logger.info("✅ Frontend: All data refreshed");
    ...
    onError: (error) => {
      logger.error("❌ Frontend: Mutation error:", error);
      setIsChangingCompany(false);
      ...
      <DropdownMenu open={isActive} onOpenChange={setActive}>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" size="sm" className="h-9">
            ...
            <span className="ml-2 font-semibold truncate text-muted-foreground text-sm">
              No Company
            </span>
  ...
  return (
    <DropdownMenu open={isActive} onOpenChange={setActive}>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="sm" className="h-9" disabled={isChangingCompany}>
          <Avatar className="h-6 w-6 rounded-lg">
          ...
          <span className="ml-2 font-semibold truncate text-sm max-w-[120px]">
        ...
        {sortedCompanies.map((company) => (
          ...
          >
            <Avatar className="h-8 w-8 rounded-lg">
              {company.logoUrl ? (
                <AvatarImage src={company.logoUrl} alt={company.name} className="rounded-lg" />
              ) : null}
              <AvatarFallback className="bg-black text-white text-xs font-semibold rounded-lg">
            ...
            {company.id === selectedId && <div className="h-2 w-2 rounded-full bg-primary" />}
          </DropdownMenuItem>
        ))}
        {isAdmin && (
          <>
