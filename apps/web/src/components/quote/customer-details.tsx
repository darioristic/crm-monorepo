...
export function CustomerDetails() {
  ...
  const handleOnChange = (newContent?: JSONContent | null) => {
    ...
    if (!newContent || !hasContent(newContent)) {
      ...
      try {
        const firstLine = (newContent?.content?.[0]?.content?.[0]?.text || "").trim();
        if (firstLine) {
          setValue("customerName", firstLine, {
            shouldValidate: true,
            shouldDirty: true,
          });
        }
  ...
  useEffect(() => {
    async function applySelection() {
      ...
      if (selectedType === "organization") {
        ...
        if (!c) {
          // Immediately set IDs so submit passes, then enrich details after fetch
