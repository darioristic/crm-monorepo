...
export function ProductAutocomplete({
	...
	const { data: productsData, refetch } = useApi(
	...
	// Filter products based on input - instant client-side filtering
	...
	}, [value, currentPrice, currentUnit, currentProductId, currency, setValue, index, refetch]);
				...
				switch (e.key) {
					case "ArrowDown":
						...
						setSelectedIndex((prev) =>
							prev === -1 ? 0 : prev < filteredProducts.length - 1 ? prev + 1 : 0
						);
						return;
					case "ArrowUp":
						e.preventDefault();
						e.stopPropagation();
						setSelectedIndex((prev) =>
							prev === -1 ? filteredProducts.length - 1 : prev > 0 ? prev - 1 : filteredProducts.length - 1
						);
						return;
					case "Enter":
						e.preventDefault();
	...
	// Exact placeholder logic from midday-main
		...
		<div ref={containerRef}>
			<input
				...
				className={cn(
					...
					"text-primary leading-[18px] font-mono",
			...
			{showSuggestions && !currentProductId && filteredProducts.length > 0 && (
				<ul
					id={listboxId}
					role="listbox"
