...
export function PdfViewer({ url, maxWidth }: PdfViewerProps) {
  ...
  function onPassword(callback: (password: string | null) => void, reason: number): void {
    switch (reason) {
    ...
    return (
      <div className="flex flex-col w-full h-full overflow-hidden items-center justify-center p-8">
        <Alert className="max-w-md">
          <AlertDescription>
            This PDF is password protected and cannot be viewed without the correct password.
          </AlertDescription>
        </Alert>
      </div>
    );
          ...
          <div className="absolute inset-0 flex items-center justify-center p-8">
            <div className="max-w-md w-full space-y-6 text-center">
              <div className="space-y-1">
                <h3 className="text-[#878787]">This document is password protected.</h3>
                <p className="text-xs text-[#878787]">Please enter the password below.</p>
              </div>
