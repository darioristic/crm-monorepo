...
registerAuthRoute("GET", "/api/v1/auth/csrf-token", async (request) => {
  ...
  try {
    const { getCsrfToken, setCsrfCookie } = await import("../middleware/csrf");
    const { env } = await import("../config/env");

    const token = await getCsrfToken(auth.sessionId);

    // Return token in response body
    const response = json(successResponse({ csrfToken: token }));

    // Also set as cookie for convenience
    return setCsrfCookie(response, token, env.NODE_ENV === "production");
  } catch (error) {
    logger.error({ error }, "Error generating CSRF token");
    return json(errorResponse("INTERNAL_ERROR", "Failed to generate CSRF token"), 500);
  }
});
...
export async function handleRequest(request: Request, url: URL): Promise<Response> {
  ...
  for (const route of sortedRoutes) {
    ...
    try {
      ...
      if (!response) {
        return json(errorResponse("INTERNAL_ERROR", "No response from handler"), 500);
      }
      // Allow binary responses for file download endpoints
      const contentType = response.headers.get("Content-Type") || "";
      const isJson = contentType.includes("application/json");
      const isBinaryAllowed =
      ...
      if (
        ...
        return json(errorResponse("INTERNAL_ERROR", "Invalid response format"), 500);
      }
      return response;
    } catch (error) {
