...
const USER_PERMISSIONS: PermissionRule[] = [
  // Users - can only read themselves
  {
    action: "read",
    resource: "users",
    condition: (auth, resourceId) => auth.userId === resourceId,
  },
  {
    action: "update",
    resource: "users",
  ...
  // Companies - can only read their own company
  {
    action: "read",
    resource: "companies",
    condition: (auth, resourceId) => auth.companyId === resourceId,
  },
 ...
 */
export function canAccessTask(auth: AuthContext, task: { assignedTo?: string }): boolean {
  if (auth.role === "tenant_admin" || auth.role === "superadmin") return true;
...
function forbiddenResponse(action: Action, resource: Resource): Response {
  return new Response(
    JSON.stringify(
      errorResponse("FORBIDDEN", `You don't have permission to ${action} ${resource}`)
    ),
    {
      status: 403,
