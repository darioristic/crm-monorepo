...
const FIRST_NAMES = [
  ...
  "Alexander", "Aubrey", "Nicholas", "Zoey", "Robert"
];
...
const LAST_NAMES = [
  ...
  "Campbell", "Mitchell", "Carter", "Roberts", "Parker"
];

const POSITIONS = [
  "CEO", "CTO", "CFO", "COO", "VP Sales", "VP Marketing", "Director", "Manager",
  "Senior Manager", "Head of Operations", "Business Development", "Account Manager",
...
const CITIES = [
  ...
  "Šabac", "Užice", "Sombor", "Požarevac", "Pirot", "Zaječar"
];

const STREETS = [
  "Knez Mihailova", "Terazije", "Kralja Milana", "Bulevar Kralja Aleksandra",
  "Nemanjina", "Vuka Karadžića", "Masarikova", "Bulevar Revolucije",
  "Trg Republike", "Obilićev venac", "Kosovska", "Bulevar Despota Stefana"
];
...
export async function seedCustomerData() {
  ...
  try {
    ...
    if (customerCompanies.length === 0) {
      console.log("⚠️  No customer companies found. Creating some first...");
    ...
    console.log("👥 Creating 50 customer contacts...");
    ...
    console.log("🏢 Creating 3 account companies for admin switching...");
    ...
    console.log(`✅ Created ${accountCompanies.length} account companies:`, accountCompanies.map(c => c.name));
    ...
    if (adminUser.length > 0) {
      ...
      console.log(`👤 Adding admin user (${admin.email}) to all account companies...`);
      ...
      for (const company of accountCompanies) {
        ...
        if (existing.length === 0) {
          ...
          `;
          console.log(`  ✓ Added admin to ${company.name}`);
        } else {
          console.log(`  ⏭️  Admin already member of ${company.name}`);
        }
      }
    } else {
    ...
    console.log("✅ Customer data seed completed!");
    return {
