  ...
  async update(id: string, data: Partial<Contact>): Promise<Contact> {
    const result = await db`
      UPDATE contacts SET
        ...
        company = COALESCE(${data.company ?? null}, company),
        position = COALESCE(${data.position ?? null}, position),
        jmbg = COALESCE(${data.jmbg ?? null}, jmbg),
...
function mapContact(row: Record<string, unknown>): Contact {
  return {
    ...
    jmbg: row.jmbg as string | undefined,
    isFavorite: (row.is_favorite as boolean | undefined) ?? undefined,
    notes: row.notes as string | undefined,
...
function mapDeal(row: Record<string, unknown>): Deal {
  return {
    ...
    expectedCloseDate: row.expected_close_date ? toISOString(row.expected_close_date) : undefined,
    actualCloseDate: row.actual_close_date ? toISOString(row.actual_close_date) : undefined,
...
export { productCategoryQueries, productQueries } from "./products";
