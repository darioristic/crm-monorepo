import type { Invoice, Quote, DeliveryNote, User } from "@crm/types";
...
const templates: Record<
	TemplateType,
	(data: Record<string, unknown>) => EmailTemplate
> = {
...
class EmailService {
	...
	async send(
		...
		if (!this.enabled) {
			console.log(`📧 [DEV] Would send email to ${options.to}: ${subject}`);
		...
		try {
			...
			console.log(`📧 Sending email to ${options.to}: ${subject}`);
