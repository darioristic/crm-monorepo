import type { Company, Invoice, Quote } from "@crm/types";
...
class ERPClient {
  ...
  constructor() {
    ...
    if (!this.enabled) {
      logger.info("🔌 ERP integration disabled (no API credentials configured)");
    }
  }
  ...
  private async request<T>(
    ...
    if (!this.enabled) {
      logger.info(`🔌 [DEV] ERP ${method} ${endpoint}`);
    ...
    try {
      ...
      if (retryCount < this.config.retries && this.isRetryableError(error)) {
        logger.info(`Retrying ERP request (${retryCount + 1}/${this.config.retries})...`);
  ...
  async getCustomer(customerId: string): Promise<ERPResponse<ERPCustomer>> {
    // Check cache first
    const cached = await cache.get<ERPCustomer>(`erp:customer:detail:${customerId}`);
    if (cached) {
      return { success: true, data: cached };
    }

    const result = await this.request<ERPCustomer>("GET", `/customers/${customerId}`);
  ...
  async updateCustomer(
    ...
    const result = await this.request<ERPCustomer>("PUT", `/customers/${customerId}`, {
      name: company.name,
      address: company.address,
    });
  ...
  async findCustomerByExternalId(companyId: string): Promise<string | null> {
    ...
    const result = await this.request<ERPCustomer[]>("GET", `/customers?externalId=${companyId}`);
  ...
  async createInvoice(invoice: Invoice, erpCustomerId: string): Promise<ERPResponse<ERPInvoice>> {
  ...
  async getInvoice(erpInvoiceId: string): Promise<ERPResponse<ERPInvoice>> {
    const cached = await cache.get<ERPInvoice>(`erp:invoice:detail:${erpInvoiceId}`);
    if (cached) {
      return { success: true, data: cached };
    }

    const result = await this.request<ERPInvoice>("GET", `/invoices/${erpInvoiceId}`);
  ...
  async updateInvoice(
    ...
    const result = await this.request<ERPInvoice>("PUT", `/invoices/${erpInvoiceId}`, {
      status: updates.status ? this.mapInvoiceStatus(updates.status) : undefined,
  ...
  async recordPayment(
    ...
    return this.request<ERPPayment>("POST", `/invoices/${erpInvoiceId}/payments`, {
      amount,
  ...
  async findInvoiceByExternalId(invoiceId: string): Promise<string | null> {
    ...
    const result = await this.request<ERPInvoice[]>("GET", `/invoices?externalId=${invoiceId}`);
  ...
  async createQuote(quote: Quote, erpCustomerId: string): Promise<ERPResponse<ERPQuote>> {
  ...
  async updateQuote(erpQuoteId: string, updates: Partial<Quote>): Promise<ERPResponse<ERPQuote>> {
    const result = await this.request<ERPQuote>("PUT", `/quotes/${erpQuoteId}`, {
      status: updates.status ? this.mapQuoteStatus(updates.status) : undefined,
  ...
  async syncInvoices(invoices: Invoice[]): Promise<SyncResult> {
    ...
    for (const invoice of invoices) {
      try {
        ...
        const erpCustomerId = await this.findCustomerByExternalId(invoice.companyId);
        ...
        if (erpInvoiceId) {
          ...
          if (!createResult.success) {
            throw new Error(createResult.error?.message || "Failed to create invoice");
          }
        }
  ...
  async syncQuotes(quotes: Quote[]): Promise<SyncResult> {
    ...
    for (const quote of quotes) {
      try {
        const erpCustomerId = await this.findCustomerByExternalId(quote.companyId);
        ...
        if (cached) {
          ...
          if (!createResult.success) {
            throw new Error(createResult.error?.message || "Failed to create quote");
          }
        }
