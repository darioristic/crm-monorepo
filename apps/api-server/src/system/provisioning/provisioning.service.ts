...
import { tenants, locations, users, userTenantRoles } from "../../db/schema/index";
...
import { errorResponse, successResponse, generateUUID } from "@crm/utils";
...
export class ProvisioningService {
    ...
    const _statusKey = `${PROVISIONING_STATUS_KEY_PREFIX}${tenantId}`;
		...
		try {
			...
			const defaultLocation = Location.create(
				tenant.id,
				"Default Location",
				"DEFAULT",
			);
			...
			if (!adminUserResult.success || !adminUserResult.data) {
				throw new Error(
					adminUserResult.error?.message || "Failed to create admin user",
				);
			}
