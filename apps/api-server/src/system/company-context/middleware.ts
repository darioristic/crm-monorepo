...
export function requireCompanyContext(
  ...
  return async (request, url, params, auth) => {
    ...
    if (!tenantId) {
      return new Response(
        JSON.stringify(errorResponse("UNAUTHORIZED", "Tenant context required")),
        {
          status: 401,
    ...
    if (!companyId) {
      return new Response(JSON.stringify(errorResponse("BAD_REQUEST", "Company ID is required")), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    const validation = await companyContextManager.validateCompanyAccess(companyId, tenantId);

    if (!validation.allowed) {
      return new Response(
        JSON.stringify(errorResponse("FORBIDDEN", validation.reason || "Company access denied")),
        {
          status: 403,
          headers: { "Content-Type": "application/json" },
        }
    ...
    // Get company context
    const companyContext = await companyContextManager.getCompanyById(companyId, tenantId);

    if (!companyContext) {
      return new Response(JSON.stringify(errorResponse("NOT_FOUND", "Company not found")), {
        status: 404,
        headers: { "Content-Type": "application/json" },
      });
    }
