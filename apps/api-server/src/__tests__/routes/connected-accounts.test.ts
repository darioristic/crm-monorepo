...
describe("Connected Accounts Routes", () => {
  ...
  const callRoute = async (
    ...
    const route = connectedAccountRoutes.find((r) => r.method === method && r.pattern.test(path));
  ...
  describe("GET /api/v1/connected-accounts", () => {
    it("should return list of connected accounts for current company", async () => {
      vi.mocked(connectedAccountQueries.findByCompany).mockResolvedValue([mockAccount]);
      ...
      expect(connectedAccountQueries.findByCompany).toHaveBeenCalledWith(mockCompanyId);
    });
