...
describe("Notification Settings Routes", () => {
  ...
  const callRoute = async (
    ...
    const route = notificationRoutes.find((r) => r.method === method && r.pattern.test(path));
  ...
  describe("GET /api/v1/notification-settings", () => {
    it("should return user notification settings", async () => {
      ...
      expect(notificationSettingsQueries.findByUserId).toHaveBeenCalledWith(mockAuth.userId);
    });
  ...
  describe("PATCH /api/v1/notification-settings", () => {
    it("should update notification setting", async () => {
      ...
      const response = await callRoute("/api/v1/notification-settings", "PATCH", {
