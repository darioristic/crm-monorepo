...
vi.mock("../../services/sales.service", () => ({
  salesService: {
    getQuotes: vi.fn(async (_companyId: string | null) => successResponse([{ id: "q1" }])),
  },
}));

describe("Quotes Route - JSON Responses", () => {
  ...
  it("GET /api/v1/quotes returns application/json on success", async () => {
    const token = await generateJWT(mockUserId, "user", mockCompanyId, mockSessionId);
    const url = new URL(
      `http://localhost/api/v1/quotes?companyId=${mockCompanyId}&page=1&pageSize=20`
    );
  ...
  it("GET /api/v1/quotes returns application/json on server error", async () => {
    const token = await generateJWT(mockUserId, "user", mockCompanyId, mockSessionId);
    // Force error path: mock salesService to return error ApiResponse
    const { salesService } = await import("../../services/sales.service");
    (salesService.getQuotes as any).mockResolvedValueOnce(
      errorResponse("INTERNAL_ERROR", "Failure")
    );

    const url = new URL(`http://localhost/api/v1/quotes?companyId=${mockCompanyId}`);
    const request = new Request(url.toString(), {
      method: "GET",
      headers: { Authorization: `Bearer ${token}` },
    });
