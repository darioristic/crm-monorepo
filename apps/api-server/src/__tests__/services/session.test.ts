...
describe('Session Management', () => {
  ...
  describe('Session Security', () => {
    it('should store session with unique session ID', async () => {
      ...
      expect(cache.setSession).toHaveBeenNthCalledWith(1, 'session-1', sessionData1, expect.any(Number));
      expect(cache.setSession).toHaveBeenNthCalledWith(2, 'session-2', sessionData2, expect.any(Number));
    });
