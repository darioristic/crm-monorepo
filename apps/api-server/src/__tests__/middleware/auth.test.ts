  ...
  describe('Role Check Helpers', () => {
    ...
    describe('canAccessCompany', () => {
      it('should allow admin to access any company', () => {
        expect(canAccessCompany(adminAuth, 'any-company-id')).toBe(true);
        expect(canAccessCompany(adminAuth, 'different-company')).toBe(true);
      });

      it('should allow user to access their own company', () => {
        expect(canAccessCompany(userAuth, mockCompanyId)).toBe(true);
      });

      it('should deny user access to other company', () => {
        expect(canAccessCompany(userAuth, 'different-company')).toBe(false);
      });

      it('should handle user without company', () => {
        const noCompanyAuth: AuthContext = {
          userId: mockUserId,
          role: 'user',
          sessionId: mockSessionId,
        };
        expect(canAccessCompany(noCompanyAuth, mockCompanyId)).toBe(false);
      });
    });
